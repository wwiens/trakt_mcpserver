...
from models import TraktDeviceCode, TraktAuthToken, FormatHelper
...
async def test_auth_flow_integration():
    ...
    with patch('httpx.AsyncClient') as mock_client, \
        ...
        from server import start_device_auth
        ...
        from server import check_auth_status
...
async def test_search_and_checkin_integration():
    ...
    # Set up mock responses for the TraktClient
    with patch('httpx.AsyncClient') as mock_client, \
         patch.dict(os.environ, {"TRAKT_CLIENT_ID": "test_id", "TRAKT_CLIENT_SECRET": "test_secret"}):
        ...
        with patch('server.TraktClient', return_value=client):
            from server import search_shows
            ...
            from server import checkin_to_show
            checkin_result = await checkin_to_show(
                season=1,
                episode=1,
                show_id="1"
            )
...
async def test_trending_shows_integration():
    ...
    with patch('httpx.AsyncClient') as mock_client, \
        ...
        from server import get_trending_shows
        ...
        from server import fetch_trending_shows
...
async def test_show_ratings_integration():
    ...
    with patch('httpx.AsyncClient') as mock_client, \
        ...
        mock_instance.get.side_effect = [show_mock, ratings_mock, show_mock, ratings_mock]
        
        from server import get_show_ratings
        ...
        from server import fetch_show_ratings
...
async def test_error_handling_integration():
    # Set up mock responses for the TraktClient that will trigger errors
    with patch('httpx.AsyncClient') as mock_client, \
         patch.dict(os.environ, {"TRAKT_CLIENT_ID": "test_id", "TRAKT_CLIENT_SECRET": "test_secret"}):
        ...
        # Verify the resource result contains an error message
        assert "Error fetching ratings for show ID 1" in resource_result
        ...
        assert "Error fetching ratings for show ID 1" in tool_result
...
async def test_comments_integration():
    ...
    with patch('httpx.AsyncClient') as mock_client, \
        ...
        # Set up the sequence of responses
        mock_instance.get.side_effect = [show_mock, comments_mock]
        
        # Test the fetch_show_comments function directly
        with patch('server.TraktClient') as mock_client_class:
            # Configure the mock
            mock_client_class.return_value.get_show.return_value = asyncio.Future()
            mock_client_class.return_value.get_show.return_value.set_result(sample_show)
            
            mock_client_class.return_value.get_show_comments.return_value = asyncio.Future()
            mock_client_class.return_value.get_show_comments.return_value.set_result(sample_comments)
            
            # Call the function directly
            from server import fetch_show_comments
            result = await fetch_show_comments(show_id="1")
            
            # Verify the result contains the expected comment
            assert "Show: Breaking Bad" in result
            assert "testuser" in result
            assert "This show is amazing!" in result
            assert "Likes: 10 | Replies: 2" in result
            
            # Verify the client methods were called
            mock_client_class.return_value.get_show.assert_called_once_with("1")
            mock_client_class.return_value.get_show_comments.assert_called_once_with("1", limit=10, sort="newest")

@pytest.mark.asyncio
async def test_comment_error_handling_no_500():
    with patch('server.TraktClient') as mock_client_class:
        mock_client = MagicMock()
        mock_client_class.return_value = mock_client
        
        mock_client.get_comment.side_effect = Exception("Error: The requested resource was not found.")
        mock_client.get_movie.side_effect = Exception("Error: The requested resource was not found.")
        mock_client.get_movie_comments.side_effect = Exception("Error: The requested resource was not found.")
        
        from server import fetch_comment, fetch_movie_comments
        
        comment_result = await fetch_comment("999999999")
        movie_comment_result = await fetch_movie_comments("999999999")
        
        assert "Error fetching comment" in comment_result
        assert "Error fetching comments" in movie_comment_result
        assert "'str' object has no attribute 'get'" not in comment_result
        assert "'str' object has no attribute 'get'" not in movie_comment_result
