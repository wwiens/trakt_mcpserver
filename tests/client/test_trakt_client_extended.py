...
async def test_get_comment_replies():
    ...
    with patch('httpx.AsyncClient') as mock_client, \
        ...
        client = TraktClient()
        result = await client.get_comment_replies(comment_id="456", limit=10, sort="newest")
...
async def test_checkin_to_show():
    ...
    with patch('httpx.AsyncClient') as mock_client, \
        ...
        call_args = mock_client.return_value.__aenter__.return_value.post.call_args[0][0]
...
async def test_checkin_to_show_with_title():
    ...
    with patch('httpx.AsyncClient') as mock_client, \
        ...
        assert call_kwargs["json"]["sharing"]["twitter"] == True
...
async def test_checkin_to_show_not_authenticated():
    with patch('dotenv.load_dotenv'), \
        ...
        assert "Error: An unexpected error occurred: You must be authenticated to check in to a show" in result
...
async def test_checkin_to_show_missing_info():
    with patch('dotenv.load_dotenv'), \
        ...
        # Check that the function returned an error message
        assert "Error: An unexpected error occurred: Either show_id or show_title must be provided" in result
